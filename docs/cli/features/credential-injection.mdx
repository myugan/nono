...
description: Keep API keys out of the sandbox with proxy-based credential injection or environment variable injection from the system keystore or 1Password
---
...
nono: 1Password credential requires an explicit variable name.
      Use format: op://vault/item/field=MY_VAR (got 'op://vault/item/<redacted>')
```
...
```bash
nono run --profile mixed-example --env-credential -- my-app
# Child process sees:
# - DATABASE_PASSWORD=... (from 1Password via env_credentials)
# - OPENAI_API_KEY=nono_sess_... (phantom token from proxy_credentials)
```
...
| `credential_key` | Yes | - | Keystore account name (alphanumeric and underscores only) or `op://` URI for [1Password](#1password-integration) |
...
| `env_var` | Conditional | - | Explicit environment variable name for the phantom token. Required when `credential_key` is an `op://` URI (e.g., `OPENAI_API_KEY`) |
