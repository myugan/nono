...
### Disable Proxy Filtering For One Session

```bash
# Keep the profile's filesystem policy, but skip proxy filtering this run
nono run --profile claude-code --net-allow -- claude
```

`--net-allow` is a per-session override. It keeps the selected profile's filesystem and command policy, but disables proxy filtering and proxy-based credential injection for that run.
...
- **Proxy mode requires supervised execution** - The proxy runs in the unsandboxed parent process, so `nono wrap` (Direct mode) is incompatible. Use `nono run` instead.
